...
* There was a need to make the app extendable without forking it out, rather than just adding it as a dependency in any android application which wants to use the functionality but with a custom UI on top of it.
...
* The openmrs-android-sdk exposes the functionality through some methods divided into various repository classes which just need to be plugged in with the UI and view-Model.
