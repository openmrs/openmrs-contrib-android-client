...
class VisitDAO {
     ...
     * The Context.
     */
    var context: Context = OpenmrsAndroid.getInstance()!!.applicationContext

    /**
     * The Observation room dao.
     */
    private var observationRoomDAO: ObservationRoomDAO = AppDatabase.getDatabase(context).observationRoomDAO()

    /**
     * The Visit room dao.
     */
    private var visitRoomDAO: VisitRoomDAO = AppDatabase.getDatabase(context).visitRoomDAO()
     ...
     */
    val activeVisits: Observable<List<Visit>>
        get() = createObservableIO(Callable<List<Visit>> {
    ...
    fun getVisitsByPatientID(patientID: Long?): Observable<List<Visit>> {
        return createObservableIO(Callable<List<Visit>> {
            val visits: MutableList<Visit> = ArrayList()
            val visitEntities: List<VisitEntity>
            try {
    ...
    fun getActiveVisitByPatientId(patientId: Long?): Observable<Visit> {
        return createObservableIO(Callable<Visit> {
            try {
    ...
    fun getVisitsIDByUUID(visitUUID: String?): Observable<Long> {
        return createObservableIO(Callable {
            visitRoomDAO.getVisitsIDByUUID(
                visitUUID!!
            )
        })
    }
