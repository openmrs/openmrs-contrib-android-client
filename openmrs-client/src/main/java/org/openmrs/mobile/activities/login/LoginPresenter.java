...
public class LoginPresenter extends BasePresenter implements LoginContract.Presenter {
    ...
    public LoginPresenter(RestApi restApi, VisitApi visitApi, LocationDAO locationDAO, UserService userService,
        LoginContract.View loginView, OpenMRS mOpenMRS, OpenMRSLogger mLogger, AuthorizationManager authorizationManager) {
        ...
        if (NetworkUtils.isOnline()) {
            ...
            if (mOpenMRS.isUserLoggedOnline() && url.equals(mOpenMRS.getLastLoginServerUrl())) {
                if (mOpenMRS.getUsername().equals(username) && mOpenMRS.getPassword().equals(password)) {
                    ...
                    loginView.showToast(R.string.login_offline_toast_message, ToastUtil.ToastType.NOTICE);
                    loginView.userAuthenticated();
                    loginView.finishLoginActivity();
                } else {
                    loginView.hideLoadingAnimation();
    ...
    public void saveLocationsToDatabase(List<Location> locationList, String selectedLocation) {
        ...
        for (int i = 0; i < locationList.size(); i++) {
            locationDAO.saveLocation(locationList.get(i)).observeOn(Schedulers.io()).subscribe();
        }
    }
    ...
    public void loadLocations(final String url) {
        ...
        if (NetworkUtils.hasNetwork()) {
            ...
            call.enqueue(new Callback<Results<Location>>() {
                ...
                @Override
                public void onResponse(@NonNull Call<Results<Location>> call,
                        @NonNull Response<Results<Location>> response) {
                    if (response.isSuccessful()) {
            ...
            addSubscription(locationDAO.getLocations().observeOn(AndroidSchedulers.mainThread()).subscribe(locations -> {
                if (locations.size() > 0) {
