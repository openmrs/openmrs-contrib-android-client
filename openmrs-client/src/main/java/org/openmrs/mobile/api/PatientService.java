...
public class PatientService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        if (NetworkUtils.isOnline()) {
            ...
            ToastUtil.error(
                getString(R.string.activity_no_internet_connection) + getString(R.string.activity_sync_after_connection));
        }
    }
    ...
    private void fetchPatientsAndCalculateLocally(Patient patient, PatientAndMatchesWrapper patientAndMatchesWrapper)
        ...
        RestApi restApi = RestServiceBuilder.createService(RestApi.class);
        Call<Results<Patient>> patientCall = restApi.getPatients(patient.getPerson().getName().getGivenName(),
