...
public class PatientSQLiteHelper extends OpenMRSSQLiteOpenHelper {
    ...
    public void insert(SQLiteDatabase db, Patient patient) {
        ...
        try {
            db.beginTransaction();
            bindString(mPatientSQLStatement, 1, patient.getDisplay());
            bindString(mPatientSQLStatement, 2, patient.getUuid());
            bindString(mPatientSQLStatement, 3, patient.getIdentifier());
            bindString(mPatientSQLStatement, 4, patient.getGivenName());
            bindString(mPatientSQLStatement, 5, patient.getMiddleName());
            bindString(mPatientSQLStatement, 6, patient.getFamilyName());
            bindString(mPatientSQLStatement, 7, patient.getGender());
            bindLong(mPatientSQLStatement, 8, patient.getBirthDate());
            bindLong(mPatientSQLStatement, 9, patient.getDeathDate());
            bindString(mPatientSQLStatement, 10, patient.getCauseOfDeath());
            bindString(mPatientSQLStatement, 11, patient.getAge());
            bindString(mPatientSQLStatement, 12, patient.getAddress().getAddress1());
            bindString(mPatientSQLStatement, 13, patient.getAddress().getAddress2());
            bindString(mPatientSQLStatement, 14, patient.getAddress().getPostalCode());
            bindString(mPatientSQLStatement, 15, patient.getAddress().getCountry());
            bindString(mPatientSQLStatement, 16, patient.getAddress().getState());
            bindString(mPatientSQLStatement, 17, patient.getAddress().getCityVillage());
            bindString(mPatientSQLStatement, 18, patient.getPhoneNumber());
